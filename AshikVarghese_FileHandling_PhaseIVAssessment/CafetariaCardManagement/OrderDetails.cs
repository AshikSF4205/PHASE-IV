...
namespace CafetariaCardManagement
{
    ...
    public class OrderDetails
    {
         ...
         public OrderDetails(string order)
        {
            ...
            s_id = Convert.ToInt32(orderArray[0].Remove(0, 3));
            _orderId = orderArray[0];
            UserID = orderArray[1];
            OrderDate = DateTime.ParseExact(orderArray[2],"dd/MM/yyyy",null);
            TotalPrice = Convert.ToDouble(orderArray[3]);
            Status = Enum.Parse<Status>(orderArray[4]);
        }
    }
}